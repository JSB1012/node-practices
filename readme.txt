...
- 일반 의존성
    개발하고 있는 프로젝트에서 사용하는 패키지로 꼭 빌드와 배포에 포함되어애 한다.
    $[project-ex01] npm i ...
- 개발 의존성
...
3) 패키지 설치 예제
    npm i(install) ejs [local install, general dependency]
    npm i -D nodemon [local install, dev. dependency]
    npm i -g gulp    [global install, general dependency]
    npx gulp --version (설치 테스트)

3) 패키지 삭제 예제
    npm un ejs [local install, general dependency]
    npm un -g gulp [global 삭제]
...
5. Module
    1) 코어 모듈: node에서 제공해 주는 모듈9(fs, os, ...)
